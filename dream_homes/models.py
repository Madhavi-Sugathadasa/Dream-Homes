...
class Rent_Ad_Item(models.Model):
    
    def _image_path_(self, filename):
        return (conf_settings.MEDIA_PATH + '/rental_properties/{0}/{1}').format(self.id, filename)
    
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="rent_ad_user")
    weekly_price = models.FloatField()
    title = models.CharField(max_length=200)
    desc = models.CharField(max_length=4000)
    property_type = models.ForeignKey(Property_Type, on_delete=models.DO_NOTHING, related_name="rent_ad_property_type")
    address_line = models.CharField(max_length=200)
    location = models.ForeignKey(Location, on_delete=models.DO_NOTHING, related_name="rent_ad_location", default="")
    bedrooms = models.IntegerField(default=1)
    bathrooms= models.IntegerField(default=1)
    parking = models.IntegerField(default=1)
    garages = models.IntegerField(default=0)
